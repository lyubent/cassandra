...
public class RecoveryManagerTest
{
    ...
    public void testOne() throws IOException
    {
        ...
        cf = ArrayBackedSortedColumns.factory.create(KEYSPACE2, CF_STANDARD3);
        cf.addColumn(column("col2", "val2", 1L));
        rm = new Mutation(KEYSPACE2, dk.getKey(), cf);
        rm.apply();

        keyspace1.getColumnFamilyStore(CF_STANDARD1).clearUnsafe();
        keyspace2.getColumnFamilyStore(CF_STANDARD3).clearUnsafe();

        CommitLog.instance.resetUnsafe(); // disassociate segments from live CL
        CommitLog.instance.recover();

        assertColumns(Util.getColumnFamily(keyspace1, dk, CF_STANDARD1), "col1");
        assertColumns(Util.getColumnFamily(keyspace2, dk, CF_STANDARD3), "col2");
    }
    ...
    public void testRecoverCounter() throws IOException
    {
        ...
        for (int i = 0; i < 10; ++i)
        {
            cf = ArrayBackedSortedColumns.factory.create(KEYSPACE1, CF_COUNTER1);
            cf.addColumn(BufferCounterCell.createLocal(cellname("col"), 1L, 1L, Long.MIN_VALUE));
            rm = new Mutation(KEYSPACE1, dk.getKey(), cf);
            rm.apply();
        }

        keyspace1.getColumnFamilyStore(CF_COUNTER1).clearUnsafe();

        CommitLog.instance.resetUnsafe(); // disassociate segments from live CL
        CommitLog.instance.recover();

        cf = Util.getColumnFamily(keyspace1, dk, CF_COUNTER1);
    ...
    public void testRecoverPIT() throws Exception
    {
        ...
        ColumnFamily cf = Util.getColumnFamily(keyspace1, dk, CF_STANDARD1);
