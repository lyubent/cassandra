...
public class RemoveSubCellTest
{
    ...
    public void testRemoveSubColumn()
    {
        ...
        ColumnFamilyStore store = keyspace.getColumnFamilyStore(CF_SUPER1);
        Mutation rm;
        DecoratedKey dk = Util.dk("key1");

        // add data
        rm = new Mutation(KEYSPACE1, dk.getKey());
        Util.addMutation(rm, CF_SUPER1, "SC1", 1, "asdf", 0);
        rm.apply();
        ...
        // remove
        rm = new Mutation(KEYSPACE1, dk.getKey());
        rm.delete(CF_SUPER1, cname, 1);
        rm.apply();

        ColumnFamily retrieved = store.getColumnFamily(QueryFilter.getIdentityFilter(dk, CF_SUPER1, System.currentTimeMillis()));
    ...
    public void testRemoveSubColumnAndContainer()
    {
        ...
        rm = new Mutation(KEYSPACE1, dk.getKey());
        rm.deleteRange(CF_SUPER1, SuperColumns.startOf(scName), SuperColumns.endOf(scName), 1);
        rm.apply();
        ...
        QueryFilter filter = QueryFilter.getIdentityFilter(dk, CF_SUPER1, System.currentTimeMillis());
        Uninterruptibles.sleepUninterruptibly(1, TimeUnit.SECONDS);

        // remove the column itself
        rm = new Mutation(KEYSPACE1, dk.getKey());
        rm.delete(CF_SUPER1, cname, 2);
        rm.apply();
