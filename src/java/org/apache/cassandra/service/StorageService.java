...
import org.apache.cassandra.repair.AnticompactionSession;
...
public class StorageService extends NotificationBroadcasterSupport implements IEndpointStateChangeSubscriber, StorageServiceMBean
{
    ...
    */
    ...
    public int forceRepairAsync(final String keyspace, final boolean isSequential, final Collection<String> dataCenters, final boolean primaryRange, final boolean repairedAt, final String... columnFamilies)
    {
        final Collection<Range<Token>> ranges = primaryRange ? getLocalPrimaryRanges(keyspace) : getLocalRanges(keyspace);
        return forceRepairAsync(keyspace, isSequential, dataCenters, ranges, repairedAt, columnFamilies);
    }

    public int forceRepairAsync(final String keyspace, final boolean isSequential, final Collection<String> dataCenters, final Collection<Range<Token>> ranges, final boolean repairedAt, final String... columnFamilies)
    {
        ...
        if (ranges.size() > 0)
        {
            new Thread(createRepairTask(cmd, keyspace, ranges, isSequential, dataCenters, repairedAt, columnFamilies)).start();
        }
        return cmd;
    }

    public int forceRepairAsync(final String keyspace, final boolean isSequential, final boolean isLocal, final boolean primaryRange, final boolean repairedAt, final String... columnFamilies)
    {
        final Collection<Range<Token>> ranges = primaryRange ? getLocalPrimaryRanges(keyspace) : getLocalRanges(keyspace);
        return forceRepairAsync(keyspace, isSequential, isLocal, ranges, repairedAt, columnFamilies);
    }

    public int forceRepairAsync(final String keyspace, final boolean isSequential, final boolean isLocal, final Collection<Range<Token>> ranges, final boolean repairedAt, final String... columnFamilies)
    {
        if (Keyspace.SYSTEM_KS.equals(keyspace) || ranges.isEmpty())
            return 0;
        ...
        if (ranges.size() > 0)
        {
            new Thread(createRepairTask(cmd, keyspace, ranges, isSequential, isLocal, repairedAt, columnFamilies)).start();
        }
        return cmd;
    }

    public int forceRepairRangeAsync(String beginToken, String endToken, final String keyspaceName, boolean isSequential, Collection<String> dataCenters, boolean repairedAt, final String... columnFamilies)
    {
        ...
        return forceRepairAsync(keyspaceName, isSequential, dataCenters, Collections.singleton(new Range<Token>(parsedBeginToken, parsedEndToken)), repairedAt, columnFamilies);
    }

    public int forceRepairRangeAsync(String beginToken, String endToken, final String keyspaceName, boolean isSequential, boolean isLocal, boolean repairedAt, final String... columnFamilies)
    {
        Token parsedBeginToken = getPartitioner().getTokenFactory().fromString(beginToken);
        Token parsedEndToken = getPartitioner().getTokenFactory().fromString(endToken);
        ...
        logger.info("starting user-requested repair of range ({}, {}] for keyspace {} and column families {}",
                    parsedBeginToken, parsedEndToken, keyspaceName, columnFamilies);
        return forceRepairAsync(keyspaceName, isSequential, isLocal, Collections.singleton(new Range<Token>(parsedBeginToken, parsedEndToken)), repairedAt, columnFamilies);
    }
     ...
     * @throws IOException
     */
    public void forceKeyspaceRepair(final String keyspaceName, boolean isSequential, boolean isLocal, boolean repairedAt, final String... columnFamilies) throws IOException
    {
        forceKeyspaceRepairRange(keyspaceName, getLocalRanges(keyspaceName), isSequential, isLocal, repairedAt, columnFamilies);
    }

    public void forceKeyspaceRepairPrimaryRange(final String keyspaceName, boolean isSequential, boolean isLocal, boolean repairedAt, final String... columnFamilies) throws IOException
    {
        forceKeyspaceRepairRange(keyspaceName, getLocalPrimaryRanges(keyspaceName), isSequential, isLocal, repairedAt, columnFamilies);
    }

    public void forceKeyspaceRepairRange(String beginToken, String endToken, final String keyspaceName, boolean isSequential, boolean isLocal, boolean repairedAt, final String... columnFamilies) throws IOException
    {
        Token parsedBeginToken = getPartitioner().getTokenFactory().fromString(beginToken);
        Token parsedEndToken = getPartitioner().getTokenFactory().fromString(endToken);
        ...
        logger.info("starting user-requested repair of range ({}, {}] for keyspace {} and column families {}",
                    parsedBeginToken, parsedEndToken, keyspaceName, columnFamilies);
        forceKeyspaceRepairRange(keyspaceName, Collections.singleton(new Range<Token>(parsedBeginToken, parsedEndToken)), isSequential, isLocal, repairedAt, columnFamilies);
    }

    public void forceKeyspaceRepairRange(final String keyspaceName, final Collection<Range<Token>> ranges, boolean isSequential, boolean isLocal, boolean repairedAt, final String... columnFamilies) throws IOException
    {
        if (Keyspace.SYSTEM_KS.equalsIgnoreCase(keyspaceName))
            return;
        createRepairTask(nextRepairCommand.incrementAndGet(), keyspaceName, ranges, isSequential, isLocal, repairedAt, columnFamilies).run();
    }

    private FutureTask<Object> createRepairTask(final int cmd, final String keyspace, final Collection<Range<Token>> ranges, final boolean isSequential, final boolean isLocal, final boolean repairedAt, final String... columnFamilies)
    {
        ...
        return createRepairTask(cmd, keyspace, ranges, isSequential, dataCenters, repairedAt, columnFamilies);
    }

    private FutureTask<Object> createRepairTask(final int cmd, final String keyspace, final Collection<Range<Token>> ranges, final boolean isSequential, final Collection<String> dataCenters, final boolean fullRepair, final String... columnFamilies)
    {
        return new FutureTask<Object>(new WrappedRunnable()
        {
            protected void runMayThrow() throws Exception
            {
                ...
                for (Range<Token> range : ranges)
                {
                    ...
                    try
                    {
                        future = forceKeyspaceRepair(range, keyspace, isSequential, dataCenters, fullRepair, columnFamilies);
                    }
                ...
                for (RepairFuture future : futures)
                {
                    ...
                    {
                        future.get();
                        // Carry out anticompaction by supplying the anticompaction session and repaired range.
                        anticompactRepairedRanges(ActiveRepairService.instance.getAnticompactionSession(future.session.getId()),
                                                  new ArrayList<Range<Token>>(Arrays.asList(future.session.getRange())));
                    ...
                    {
                        ActiveRepairService.instance.removeFromAnticompactionSession(future.session.getId());
                    }
                }
    ...
    public RepairFuture forceKeyspaceRepair(final Range<Token> range, final String keyspaceName, boolean isSequential, Collection<String> dataCenters, boolean fullRepair, final String... columnFamilies) throws IOException
    {
        ArrayList<String> names = new ArrayList<String>();
        for (ColumnFamilyStore cfStore : getValidColumnFamilies(false, false, keyspaceName, columnFamilies))
        {
            boolean skipRepair = false;
            for(SSTableReader sstable : cfStore.getSSTables())
            {
                ...
                // skip already repaired ranges, unless the -ipr flag (fullRepair == true) is supplied via the nodetool
                if (!fullRepair && sstable.getSSTableMetadata().repairedAt != null && range.intersects(sstableRange))
                {
                    // range is contained in an sstable that has already been repaired. Skip the repair.
                    logger.info("Range " + range.toString() + " has already been repaired. Skipping repair.");
                    skipRepair = true;
                    // exit the loop, we already know this range has been repaired.
                    break;
                }
            }
    ...
    /**
     * Segregates an sstable's repaired ranges by placing them in two new sstables. A table gets marked
     * as repaired via the SSTableMetadata#repairedAt field which stores a timestamp of the repair time.
     * If the field is null, the table isn't repaired.
     * @param session An anticompaction session storing the columnfamilystore and sstables required for
     * the anticompaction.
     * @param repairedRanges
     * @throws InterruptedException, ExecutionException, IOException
     */
    public void anticompactRepairedRanges(AnticompactionSession session, Collection<Range<Token>> repairedRanges) throws InterruptedException, ExecutionException, IOException
    {
        if (repairedRanges.size() == 0)
        {
            logger.info("Cannot anti-compact an empty range.");
            return;
        }
        if (session.validatedForRepair == null || session.validatedForRepair.size() == 0)
        {
            logger.info("Cannot anti-compact 0 sstables.");
            return;
        }
        session.cfs.forceAntiCompaction(repairedRanges, session.validatedForRepair);
    }
